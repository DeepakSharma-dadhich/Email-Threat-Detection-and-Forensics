...
class EmailIngestionService:
    ...
    def ingest(
        ...
        if (
            len(raw_bytes)
            > settings.max_email_size_bytes
        ):
            raise AppError(
                (
                    "Email exceeds the configured "
                    f"{settings.max_email_size_mb} "
                    "MB limit."
                ),
                413,
                "EMAIL_TOO_LARGE",
        ...
        raw_sha256 = self.storage.sha256(
            raw_bytes
        )
